...
if __name__ == '__main__':
    ...
    train_data_poly = poly.fit_transform(train_data.iloc[:,:-1])
    test_data_poly = poly.fit_transform(test_data)
    print('完成多项式组合')
    np.c_[train_data_poly,np.asarray(train_data['target'])]
    columns = []
    for i in range(train_data_poly.shape[1]):
        col = 'V_' + str(i)
        columns.append(col)
    train_data_new = pd.DataFrame(train_data_poly,columns=columns)
    train_data_new['target'] = train_data['target']
    corr = train_data_new.corrwith(train_data_new['target'])
    corr = corr.reindex(corr.abs().sort_values().index)

    test_data_new = pd.DataFrame(test_data_poly,columns=columns)

    train_data_new = train_data_new[corr[corr.abs() > 0.1].index]
    lis = list(corr[corr.abs() > 0.1].index)[:-1]
    test_data_new = test_data_new.loc[:, lis]
    ...
    target = model.predict(test_data_new)
    print(target)
    line_model.fit(X, y)
    print(mean_squared_error(line_model.predict(X_test), y_test))
    line_target = line_model.predict(test_data_new)
    print(line_target)
